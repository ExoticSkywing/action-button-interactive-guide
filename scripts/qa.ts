...
async function runCase(engine: string, type: BrowserType, width: number, height: number) {
  ...
  try {
    ...
    await page.waitForSelector('[data-screen-state="settings"]');
    ...
    await page.waitForSelector('.viewer.metal-flowing');
    ...
    await page.waitForSelector('[data-screen-state="action"]');
    await page.waitForTimeout(760);
    ...
    await page.waitForSelector('[data-screen-state="translate"]');
    ...
    await page.waitForSelector('[data-screen-state="complete"]');
    ...
    const row = { engine, width, height, expectsTouch, initial, railCount, hudButtonCount, expectedGesture, initialCoachmark, beamInitiallyPaused, reflectionStep0, blockedClickState, practiceState, nextFlow, nextSettled, previousFlow, afterClickHidden, gestureState, learnedCoachmark, beamActive, secondGestureState, reflectionStep2, metal, restored, remainsHidden, proof, collapsed, beamPausedCollapsed, expanded, beamPausedExpanded, collapsedTransaction, collapsedSettled, restoredFocus, topControlStep, railKeyboardStep, runtime, consoleErrors, pageErrors, requestFailures, initialShot, fogShot };
    results.push(row);
    const failed =
      ...
      reflectionStep0.filter(item => item.visibility === 'visible').length !== 1 || !reflectionStep0[1].classes.includes('reflection-below') || reflectionStep0[1].opacity !== '1' || !reflectionStep0[1].background.includes('chromatic-reflection-top-80.webp') || reflectionStep0.some((item, index) => index !== 1 && (item.visibility !== 'hidden' || item.opacity !== '0')) ||
      ...
      nextFlow.direction !== 'next' || !nextFlow.flowing || !nextFlow.viewerFlowing || nextFlow.coreAnimation !== 'metal-flow-core-next' || nextFlow.streamAnimation !== 'metal-stream-next' || nextFlow.beadAnimation !== 'metal-bead-next' || nextFlow.streamOpacity <= 0 || nextFlow.beadOpacity < 0 || nextFlow.coachOpacity !== '0' ||
      nextSettled.direction !== null || nextSettled.flowing || nextSettled.viewerFlowing || nextSettled.coachOpacity !== '0' ||
      previousFlow.direction !== 'previous' || !previousFlow.flowing || previousFlow.coreAnimation !== 'metal-flow-core-previous' || previousFlow.streamAnimation !== 'metal-stream-previous' || previousFlow.beadAnimation !== 'metal-bead-previous' ||
      afterClickHidden !== 'true' || gestureState !== 'action' || secondGestureState !== 'translate' || beamActive.paused || beamActive.layers !== 3 || beamActive.z.join(',') !== '1,2,3,4' || beamActive.parity.duration !== '1.96s' || beamActive.parity.strength !== '1' || beamActive.parity.stroke !== '0.26' || beamActive.parity.inner !== '0.42' || beamActive.parity.bloom !== '0.24' ||
      ...
      restored !== 'translate' || remainsHidden !== 'true' || !proof.includes('设置完成') || collapsed !== 'collapsed' || !beamPausedCollapsed || expanded !== 'expanded' || beamPausedExpanded ||
      ...
      runtime.canvas || runtime.video || runtime.webgl || runtime.bezel.join('x') !== '1350x2760' || !runtime.stageBackground.includes(width <= 600 ? 'black-wavy-ribbons-mobile.webp' : 'black-wavy-ribbons-desktop.webp') || runtime.stageAsset.natural.join('x') !== (width <= 600 ? '1200x2133' : '1600x1262') || consoleErrors.length || pageErrors.length || requestFailures.length;
