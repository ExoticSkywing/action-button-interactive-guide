...
## Direction
Immersive Apple Product Viewer, not a white landing page with a demo component. One neutral-black digital-ribbon stage contains the local navigation, official device, functional icon rail, connector, close control, and one tutorial HUD.
...
## Visual system
- Stage: static neutral-black digital ribbon artwork over `#000`, using separate mobile and desktop crops. The stage never follows or samples uploaded screen colors. Runtime adds no Canvas, WebGL, JavaScript, animation, filter, or video; see `THIRD_PARTY_NOTICES.md`.
...
- Motion: each real step uses one direction-aware 460ms Metal transfer. Next and previous are vertical mirrors; reduced motion bypasses the transfer.
- Rail performance: the 460ms transition has one owner. Repeated input keeps only one latest adjacent intent; it never restarts active Metal motion or skips a tutorial step. Stale screen timers are cancelled, four phone screens are parsed once and reused, forced layout reads are forbidden, and the HUD beam pauses during the transaction.
...
- HUD edge motion uses `border-beam` 1.4.0 `md/dark/colorful`. Normal motion matches the official Main Large demo at 1.96s and strength 1. Reduced motion uses a static mono edge.
